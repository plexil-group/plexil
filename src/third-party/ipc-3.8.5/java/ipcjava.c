			       ...
			       void *handlerNum)
{
  ...
  (*env)->CallStaticVoidMethod(env, ipcClass, msgHandlerID, (jlong)handlerNum,
			       (jlong)msgInstance, (jlong)callData);
				       ...
				       BYTE_ARRAY callData, void *handlerNum)
{
  ...
  (*env)->CallStaticVoidMethod(env, ipcClass, queryNotifyHandlerID, 
			       (jlong)handlerNum, (jlong)msgInstance, 
			       (jlong)callData);
  /* Does not seem to work under my version of Linux (or else I am doing
     something really wrong here */
  //(*pJavaVM)->DetachCurrentThread(pJavaVM);
				 ...
				 unsigned long scheduledTime)
{
  ...
  (*env)->CallStaticVoidMethod(env, ipcClass, timerHandlerID,
			       (jlong)handlerNum, (jlong)currentTime,
			       (jlong)scheduledTime);
  /* Does not seem to work under my version of Linux (or else I am doing
     something really wrong here */
  //(*pJavaVM)->DetachCurrentThread(pJavaVM);
...
static void ipcJavaFdHandler (int fd, void *handlerNum)
{
  ...
  (*pJavaVM)->AttachCurrentThread(pJavaVM, (void **)(void *)&env, NULL);
  (*env)->CallStaticVoidMethod(env, ipcClass, fdHandlerID, (jlong)fd);
  /* Does not seem to work under my version of Linux (or else I am doing
     something really wrong here */
  //(*pJavaVM)->DetachCurrentThread(pJavaVM);
}
...
static void ipcJavaConnectHandler (const char *moduleName, void *clientData)
{
  ...
  (*env)->CallStaticVoidMethod(env, ipcClass, connectHandlerID,
			       ...
			       ((BOOLEAN)(jlong)clientData ? JNI_TRUE : JNI_FALSE));
...
Java_ipc_java_IPC_IPC_1msgInstanceFormatter (JNIEnv *env, jclass theClass,
				    jlong msgInstance)
{
  return (jlong)IPC_msgInstanceFormatter((MSG_INSTANCE)msgInstance);
}
			   ...
			   jint handlerNum)
{
  ...
  retVal = IPC_queryNotify(cmsgName, (int)length, (BYTE_ARRAY)byteArray,
			   ipcJavaQueryNotifyHandler, (void *)(jlong)handlerNum);
  (*env)->ReleaseStringUTFChars(env, msgName, cmsgName);
			     ...
			     jlong timeoutMSecs)
{
  ...
  if (retVal == IPC_OK) {
    ...
    jfieldID formatterFieldID = (*env)->GetFieldID(env, queryResponseClass,
						   "formatter", "J");
    (*env)->SetLongField(env, response, byteArrayFieldID, (jlong)replyHandle);
    (*env)->SetLongField(env, response, formatterFieldID, (jlong)replyFormat);
  }
...
{
  return (jlong)formatChoosePtrFormat((CONST_FORMAT_PTR)formatter,
...
Java_ipc_java_formatters_formatFormatArray (JNIEnv *env, jclass theClass,
				     jlong formatter)
{
  return (jlong)formatFormatArray((FORMAT_PTR)formatter);
}
...
{
  return (jlong)formatFormatArrayItem((FORMAT_ARRAY_PTR)formatArray, (int)n);
}
...
Java_ipc_java_formatters_formatFormatArrayItemInt (JNIEnv *env, jclass theClass,
					  jlong formatArray, jint n)
{
  return (jint)(long)formatFormatArrayItem((FORMAT_ARRAY_PTR)formatArray,
					   (int)n);
}
...
{
  return (jlong)findNamedFormat((FORMAT_PTR)format);
}
...
{
  return (jlong)x_ipcMalloc(length);
}
