...
public class ForNodeModel extends NodeModel {
	...
	public String decompile(int indentLevel) {
		String ret = indent(indentLevel) + "for( " + getChild("DeclareVariable").decompile(0) + " ";
		BaseModel aux = null;
		...
		for( BaseModel grandchild : aux.getChildren() ) {
			if( grandchild instanceof ConditionModel ) {
				if( ((ConditionModel) grandchild).getType().equals("Skip") ) {
					if( grandchild.hasChild("NOT") ) {
						condition = grandchild.getChild("NOT").getChildren().get(0).decompile(0);
					} else {
