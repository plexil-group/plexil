		      ...
		      META_MASK)
	{
	    public void actionPerformed(ActionEvent e) 
	    {
			...
			Luv.getLuv().getLuvStateHandler().stopExecutionState();
                ...
                File plan = new File(Luv.getLuv().getCurrentPlan().getPlanName());
                if (plan.exists())
                {
                    Luv.getLuv().getFileHandler().loadPlan(new File(Luv.getLuv().getCurrentPlan().getPlanName()));
                    Luv.getLuv().getStatusMessageHandler().showStatus("Plan \"" + Luv.getLuv().getCurrentPlan().getPlanName() + "\" loaded", 1000);
                    Luv.getLuv().getLuvStateHandler().openPlanState(); 
                }
                else
                {
